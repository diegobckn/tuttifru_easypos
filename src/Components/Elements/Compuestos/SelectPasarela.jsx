...
const SelectPasarela = ({
  ...
  const loadList = async()=>{
    try {
      ...
      setSelectList([
        { id: 1, tipoPasarela: "GETNET ,80 MM, 83948394" },
        { id: 2, tipoPasarela: "GETNET ,57 MM, 98902323" },
        { id: 3, tipoPasarela: "MERCADOPAGO , 57 MM, 00945845 " },
      ]);
  ...
  const setByString = (valueString)=>{
    selectList.forEach((item)=>{
      if(item.tipoPasarela == valueString){
        setSelected(item.id)
    ...
    <>
      ...
      >
        ...
        {selectList.map((selectOption,ix) => (
          <MenuItem
            key={ix}
            value={selectOption.tipoPasarela}
          >
            {selectOption.tipoPasarela}
          </MenuItem>
        ))}
      </Select>
    </>
