...
import SeleccionarRepartidorApp from "../ScreenDialog/SeleccionarRepartidorApp";
...
export default ({
  ...
  const [showSelecRepartidor, setShowSelecRepartidor] = useState(false)
  ...
  const cargarPedidosProgramados = () => {
    showLoading("Cargando pedidos programados...")
    Atudepa.obtenerPedidosProgramados((resp) => {
  ...
  const [repartidorSelec, setRepartidorSelec] = useState(null);
  ...
  const cambiarEstado = (nuevoEstado, idEspecial = 0) => {
    ...
    Atudepa.cambiarEstadosPedidos(ids, nuevoEstado, () => {
      ...
      cargarPedidosProgramados()
      hideLoading();
    }, showMessageLoading)
  }


  const asignar = (idEspecial = 0) => {
    var ids = []
    if (!idEspecial) {
      isCheck.forEach((index) => {
        ids.push(pedidos[index].id)
    ...
    console.log("a los ids", ids)
    showLoading("asignando pedidos...")
    Atudepa.asignarPedidosProgramados(ids, repartidorSelec, () => {
      setIsCheck([])//reseteamos los checks
      cargarPedidosProgramados()
      hideLoading();
    }, showMessageLoading)
  ...
  const iniciarTodo = async () => {
    ...
    setIsCheck([])//reseteamos los checks
    Atudepa.nuevoPedidoProgFuncion = (peds) => {
      console.log("nuevos pedidos2 programados", peds)
      // Atudepa.checkNuevosPedidosProg = false
      ...
      //   Atudepa.checkNuevosPedidosProg = true
      //   cargarPedidosProgramados()
      // })


      var txt = "Cargando Nuevo pedido programado";
      if (peds.length > 1) {
        txt = "Cargando Nuevos pedidos programados"
      }
      setTxtNuevos(txt)
      cargarPedidosProgramados()
    }

    Atudepa.iniciarCicloProgramados()
    setIniciado(true)
  }
  ...
  return visible ? (<Grid container spacing={2} >
    ...
    <Grid item xs={12} sm={12} md={12} lg={12}>
      {repartidorSelec ? (
        <>
          <Typography>Repartido Seleccionado {repartidorSelec.full_name}</Typography>
          <SmallButton textButton={"Cambiar repartidor"} actionButton={() => {
            setShowSelecRepartidor(true)
          }} />
        </>
      ) : (
        <SmallButton textButton={"Seleccionar repartidor"} actionButton={() => {
          setShowSelecRepartidor(true)
        }} />
      )}
    </Grid>
        ...
        <Grid item xs={12} sm={12} md={3} lg={3}>
          ...
          {repartidorSelec && (
            <SmallSecondaryButton
              textButton={"Asignar a " + repartidorSelec.full_name}
              actionButton={() => {
                if (isCheck.length < 1) {
                  showMessage("Seleccionar algun pedido")
                  return
                }
                // cambiarEstado(EstadosPedidosApp.REPARTIENDO)
                asignar()
              }}
            />
          )}
          ...
          <SeleccionarRepartidorApp
            openDialog={showSelecRepartidor}
            setOpenDialog={setShowSelecRepartidor}
            onSelect={(x) => { setRepartidorSelec(x) }} />
        </Grid>
      )
    }
  </Grid >) : (<>
